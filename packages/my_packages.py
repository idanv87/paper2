...
def Gauss_zeidel(A, b, x, theta=1, iter=2):
    ...
    return [x, it_count, np.linalg.norm(A@x-b)/np.linalg.norm(b)]      
import time
def Gauss_zeidel2(U,L, b, x):

    x=spsolve(L,b-U@x)
    # x=scipy.sparse.linalg.spsolve_triangular(L,b-U@x,overwrite_A=True, overwrite_b=True)

    return x
    # x, exit_code = lgmres(L, b-U@x,tol=1e-12, maxiter=4000)
    # return x
    # return spsolve_triangular(L,b-U@x, lower=True)
...
def gmres2(A, b, x0, nmax_iter, restart=None):
    e=0
